...
def vol(ticker, start, end):
    ...
    # 2️⃣ Kalau hasil NaN → fallback ke 30 hari sebelum end_date
    ...
    # 3️⃣ Kalau masih NaN → pakai std dari seluruh return
    if pd.isna(vol_value):
...
def predict_headline_with_ticker(
    ...
    explain : bool
        True → tampilkan highlight HTML LIME
    ...
    # 1️⃣ Ambil data numeric features
    ...
    print(f"📈 Volatility: {vol_value:.6f}, Avg price: {avg_val:.2f}, Price change: {price_change_pct:.4f}")

    # 6️⃣ Optional LIME Explanation
